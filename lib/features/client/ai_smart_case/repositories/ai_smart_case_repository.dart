...
class AISmartCaseRepository {
  ...
  static Options get _aiOptions =>
      Options(receiveTimeout: _aiTimeout, sendTimeout: _aiTimeout);
  ...
  Future<AISmartCaseSessionResponse> analyzeSmartCase({
    ...
    if (response.statusCode == 200 && response.data != null) {
      ...
      throw Exception(
        response.data?['message'] ?? 'Failed to analyze smart case.',
      );
    }
  }
  ...
  Future<AISmartCaseSessionResponse> answerQuestions({
    ...
    final response = await DioClient.dio.post(
      ...
      data: {'sessionId': sessionId, 'answers': answers},
      options: _aiOptions,
    );
