...
class _AICaseReviewScreenState extends ConsumerState<AICaseReviewScreen> {
  ...
  void initState() {
    ...
    _descController = TextEditingController(text: analysis?.caseDescription ?? '');
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                            ...
                            child: OutlinedButton(
                              onPressed: () {
                                if (duplicate.existingCaseId != null) {
                                  context.push('${RouteNames.caseProgress}/${duplicate.existingCaseId}');
                                }
                              },
                              ...
                              child: const Text("Open Existing", style: TextStyle(fontSize: 11)),
                            ),
                          ),
              ...
              // Field 1: Title
              const Text("Case Title", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13.5)),
              const SizedBox(height: 6),
              TextField(
              ...
              // Field 2: Description
              const Text("Detailed Legal Description", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13.5)),
              const SizedBox(height: 6),
              TextField(
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text("Legal Category", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13.5)),
                        const SizedBox(height: 6),
                        Container(
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text("Priority", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13.5)),
                        const SizedBox(height: 6),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12),
                          decoration: BoxDecoration(
              ...
              if ((analysis?.detectedTimeline ?? []).isNotEmpty) ...[
                const Text("Extracted Timeline", style: TextStyle(color: AppColors.primaryGold, fontWeight: FontWeight.bold, fontSize: 14)),
                    ...
                    children: [
                      for (var item in analysis!.detectedTimeline)
                        Padding(
                          ...
                          child: Row(
                            children: [
                              const Icon(Icons.access_time_filled, color: AppColors.primaryGold, size: 14),
                              const SizedBox(width: 8),
              ...
              if (lawyers.isEmpty)
                Container(
                  ...
                  child: const Text("No specific lawyer match found. Your case will be posted to all open advocates.", style: TextStyle(color: AppColors.mutedText, fontSize: 12)),
                )
              else
                SizedBox(
                  height: 160,
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final isSelected = state.selectedLawyer?.userId == lawyer.userId;

                      return GestureDetector(
                        onTap: () {
                          ref.read(aiSmartCaseProvider.notifier).selectLawyer(lawyer);
                        },
                        child: Container(
                          width: 240,
                          ...
                          decoration: BoxDecoration(
                            color: isSelected ? const Color(0xFF1E1035) : const Color(0xFF0F1424),
                            borderRadius: BorderRadius.circular(16),
                            border: Border.all(
                              ...
                              Row(
                                children: [
                                  CircleAvatar(
                                    ...
                                    child: Text(
                                      lawyer.name.isNotEmpty ? lawyer.name[0] : "A",
