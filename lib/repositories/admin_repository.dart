...
class AdminRepository {
  ...
  // 2. Clients
  Future<List<AdminClientSummary>> getClients({
    String? search,
    String? status,
  }) async {
    try {
  ...
  // 3. Lawyers
  Future<List<AdminLawyerSummary>> getLawyers({
    String? search,
  ...
  // 9. Support Tickets
  Future<List<IssueModel>> getSupportTickets({
    String? status,
    String? search,
  }) async {
    try {
  ...
  Future<bool> broadcastNotification(
    ...
    try {
      final response = await DioClient.dio.post(
        ...
        data: {'title': title, 'message': message, 'targetRole': targetRole},
      );
      return response.data != null && response.data['success'] == true;
    } catch (_) {
      return false;
